...
func (r *Repository) CreateTweet(tweet *models.Tweet) error {
    query := `INSERT INTO tweets (user_id, username, content, likes, created_at)
              VALUES (?, ?, ?, ?, ?)`

    now := time.Now()
    result, err := r.db.Exec(query, tweet.UserID, tweet.Username, tweet.Content, 0, now)
    if err != nil {
        return err
    }
...
func (r *Repository) GetAllTweets() ([]*models.Tweet, error) {
    query := `SELECT id, user_id, username, content, likes, created_at 
    ...
    for rows.Next() {
        ...
        err := rows.Scan(&tweet.ID, &tweet.UserID, &tweet.Username,
            &tweet.Content, &tweet.Likes, &tweet.CreatedAt)
        if err != nil {
            return nil, err
        }
